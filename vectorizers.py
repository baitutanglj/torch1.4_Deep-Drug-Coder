...
class SmilesVectorizer(object):
    ...
    :parameter augment: Enumerate(列出) the SMILES during transform
    :parameter canonical: use canonical(典範) SMILES during transform (overrides enum)
...
# import numpy as np
# v = np.array([[[0.1,0.2,0.3,0.4],[0.1,0.5,0.1,0.2]]])
# v=v[v.sum(axis=2)==1]
# v.argmax(axis=1)
